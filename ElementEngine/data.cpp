        ...
        const auto prefix_len = [&]() -> std::size_t {
            if (list->offset != static_cast<std::size_t>(-1)) {
                ...
                if (list->sdt == "SKILLTOME_SUB_TYPE") {
                    ...
                    throw std::runtime_error(QString("unknown structure of '%1' prefix").arg(list->caption.c_str()).toStdString());
                }
            }
        }();
...
data::data(const char* path) : _path(path) {
    ...
    if (!ifs) {
        throw std::runtime_error(QString("cannot open elements.data at '%1'").arg(_path.c_str()).toStdString());
    }

    if (!ifs.read(reinterpret_cast<char*>(&_version), sizeof(_version))) {
        throw std::runtime_error(QString("cannot read version from first 2 bytes of '%1'").arg(_path.c_str()).toStdString());
    }
}
